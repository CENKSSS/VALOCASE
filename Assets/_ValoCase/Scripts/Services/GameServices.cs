...
namespace ValoCase.Services
{
    ...
    public sealed class UpgradeService : IUpgradeService
    {
        ...
        // Value ratio raised to a >1 power so upgrades are globally harder than a flat
        // ratio (e.g. ratio 0.5 → 0.25, 0.67 → ~0.45). Single global difficulty knob.
        const float UpgradeChanceExponent = 2f;

        /// <summary>
        /// Value-ratio chance (totalInputValue / targetValue), made stricter by an
        /// exponent and clamped to 0..1. More expensive targets yield a lower chance.
        /// </summary>
        public float ComputeValueChance(int totalInputValue, int targetValue)
        {
            if (totalInputValue <= 0 || targetValue <= 0) return 0f;
            float ratio = (float)totalInputValue / targetValue;
            return Mathf.Clamp01(Mathf.Pow(Mathf.Clamp01(ratio), UpgradeChanceExponent));
        }
        ...
        public bool TryUpgradeMultiTarget(IReadOnlyList<SkinDefinitionSO> inputs, IReadOnlyList<SkinDefinitionSO> targets, out bool success)
        {
            success = false;
            if (inputs == null || inputs.Count == 0 || targets == null || targets.Count == 0 || _inventory == null) return false;
            ...
            foreach (var t in targets)
            {
                ...
                targetTotal += t.VpValue;
            }

            // Rule: target total VP must be strictly greater than the input total.
            if (targetTotal <= inputTotal) return false;

            foreach (var kv in required)
                if (_inventory.GetQuantity(kv.Key) < kv.Value) return false;

            var chance = ComputeValueChance(inputTotal, targetTotal);
            success = _provider.GenerateUpgrade(inputs, targets[0], chance).Success;
            ...
            if (success)
                foreach (var t in targets)
                    _inventory.AddSkin(t, out _);

            _save?.Save();
            OnUpgradeResolved?.Invoke(inputs[inputs.Count - 1], targets[targets.Count - 1], success);
            return true;
        }
    }
